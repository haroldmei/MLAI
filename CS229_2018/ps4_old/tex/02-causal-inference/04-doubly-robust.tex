...
\item \subquestionpoints{4} Please show that the doubly robust estimator is equal to $\E_{\substack{s\sim p(s) \\ a \sim \pi_1(a, x)}} R(s, a)$ when $\hat{\pi}_0 = \pi_0$
\item \subquestionpoints{3} Please show that the doubly robust estimator is equal to $\E_{\substack{s\sim p(s) \\ a \sim \pi_1(a, x)}} R(s, a)$ when $\hat{R}(s, a) = R(s, a)$
\end{enumerate}