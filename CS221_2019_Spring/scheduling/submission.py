...
def create_nqueens_csp(n = 8):
    ...
    # name variables as x_1, x_2, ..., x_n
    variables = ['x%d'% (i + 1) for i in range(n)]
    ...
    for i in range(length):
        for j in range(i + 1, length):
            ...
            lambda x, y : (x != y) and (abs(x - y) != j - i))
    # END_YOUR_CODE
    return csp
...
class BacktrackingSearch():
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            # BEGIN_YOUR_CODE (our solution is 7 lines of code, but don't worry if you deviate from this)
            minNum = float('inf')   #only for first time comparison
            minIdx = -1
            for i in range(len(self.csp.variables)):
                var = self.csp.variables[i]
                if var not in assignment: 
                    ...
                    if length < minNum:
                        minIdx = i
                        minNum = length
            return self.csp.variables[minIdx]
            # END_YOUR_CODE
