...
sa = [np.ones([NUM_STATES, NUM_STATES], int), np.ones([NUM_STATES, NUM_STATES], int)]
psa = [sa[i]/np.sum(sa[i], axis = 0) for i in range(2)]
R = np.zeros([NUM_STATES])
V = np.random.rand(NUM_STATES)/10
###### END YOUR CODE ######
...
while consecutive_no_learning_trials < NO_LEARNING_THRESHOLD:
    ...
    v = [V.dot(psa[i]) for i in range(2)]
    action = 0 if v[0][state] > v[1][state] else 1
    ###### END YOUR CODE ######
    ...
    # !!! rewards are actually invariant. !!!
    ...
    # !!! rewords are all '0' except the last one !!!
    # record the number of time `new_state` was reached
    # !!! Don't mix it with sa[action][state][new_state] !!!
    # !!! the last subscript is for column vector, each column vector is the statistics transiting FROM the current 'state' !!!
    # !!! the middle subscript is for row vector, denotes the statistics from 'state' to 'new_state' !!!
    sa[action][new_state][state] += 1
    ###### END YOUR CODE ######
    ...
    if new_state == NUM_STATES - 1:
        ...
        #  pole falls, update module (psa and rewards, but since rewards are invariant, no need to update it)
        psa = [sa[i]/np.sum(sa[i], axis = 0) for i in range(2)]
        ###### END YOUR CODE ######
        ...
        # Synchronous update with state/psa vectors
        iteration = 0
        prev_value = np.zeros([NUM_STATES])
        while np.max(np.abs(prev_value - V)) > TOLERANCE:
            prev_value = V
            v1 = [V.dot(psa[i]) for i in range(2)]
